...
export default function OAuth() {
  ...
  const handleGoogleClick = async () => {
    ...
    try {
      ...
      const userEmail = resultsFromGoogle.user.email;

      // Validate email
      if (
        !userEmail.endsWith('@bitsathy.ac.in') &&
        userEmail !== 's.vishnu6678@gmail.com'
      ) {
        alert("Only college email addresses (@bitsathy.ac.in) allowed.");
        return; // Stop further execution
      }

      // Proceed with the API call if the email is valid
